...
def addPost(request, post_id=None):
    if post_id:
        ...
        if request.method == 'POST':
            ...
            form = PostForm()
    content = {'form': form, 'post': post if post_id else None, 'post_id': post_id}
    return render(request, 'admin/add-post.html', content)

def addPost(request, post_id=None):
    post = get_object_or_404(Posts, id=post_id) if post_id else None
    
    # Nếu có một bài viết hiện tại (cập nhật bài viết), sử dụng nó làm instance. Ngược lại, không sử dụng instance.
    ...
    content = {'form': form, 'post': post, 'post_id': post_id}
    return render(request, 'admin/add-post.html', content)
