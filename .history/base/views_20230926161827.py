...
def detailPage(request, post_id):
    ...
    # Tìm tiêu đề trong nội dung (ví dụ: tìm thẻ h1)
    title_tag = soup.find('h1')
    
    # Trích xuất tiêu đề (nếu tìm thấy)
    if title_tag:
        title_text = title_tag.text
    else:
        title_text = "Tiêu đề mặc định nếu không tìm thấy"
    
    paragraphs = post.content.split('\n')
    context = {'user': user, 'post': post, 'paragraphs': paragraphs, 'title_text': title_text}
