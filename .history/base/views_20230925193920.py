...
def updatePost(request, post_id):  # Chú ý tham số post_id
    post = get_object_or_404(Posts, id=post_id)  # Lấy đối tượng cần cập nhật dựa trên ID
    if request.method == 'POST':
        form = PostForm(request.POST, request.FILES, instance=post)  # Truyền instance là đối tượng cần cập nhật
        if form.is_valid():
        ...
        form = PostForm(instance=post)  # Khởi tạo form với dữ liệu của đối tượng cần cập nhật
    return render(request, 'admin/add-post.html', {'form': form})
